...
from app.api.v1.schemas.common import (
    ...
    UserContentCreate,
    UserCourseCreate,
    UserSubjectCreate, BaseCommonUpdate
)
from app.db.crud.common import (
    user_course_create, user_course_update, user_subject_create, user_subject_update, user_unit_create, user_unit_update
)

common_router = APIRouter(prefix="/common", tags=["Commons"])

@common_router.post("/user-course/create/")
def create_user_course(user_course: UserCourseCreate, db: Annotated[Session, Depends(get_db)]):
    try:
...
def update_user_course(user_course_id: int, user_course: BaseCommonUpdate, db: Annotated[Session, Depends(get_db)]):
    try:
...
def create_user_subject(user_subject: UserSubjectCreate, db: Annotated[Session, Depends(get_db)]):
    try:
...
def update_user_subject(user_subject_id: int, user_subject: BaseCommonUpdate, db: Annotated[Session, Depends(get_db)]):
    try:
...
def create_user_unit(user_unit: UserUnitCreate, db: Annotated[Session, Depends(get_db)]):
    try:
...
def update_user_unit(user_unit_id: int, user_unit: BaseCommonUpdate, db: Annotated[Session, Depends(get_db)]):
    try:
