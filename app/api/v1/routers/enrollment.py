...
from app.db.crud.enrollment import course_enrollment_payment_update, course_enrollment_create, \
    fetch_user_enrollments_by_course, fetch_user_enrollments
...
from app.api.v1.schemas.enrollment import CourseEnrollmentCreate, PaymentIntentResponse, CourseEnrollmentUpdate
...
def create_enrollment_session(data: CourseEnrollmentCreate, db: Annotated[Session, Depends(get_db)]):
    try:
        ...
        if existing_enrollment and existing_enrollment.status == PaymentStatus.PAID:
            raise HTTPException(status_code=400, detail="Already enrolled to this course")
        elif existing_enrollment and existing_enrollment.status == PaymentStatus.PENDING:
            raise HTTPException(status_code=400, detail="Your payment is being processed. Please wait!")
...
def create_course_enrollment(enrollment_data: CourseEnrollmentCreate, db: Annotated[Session, Depends(get_db)]):
    try:
        ...
        return PaymentIntentResponse(client_secret=intent.client_secret, enrollment_id=payment.id)
...
async def stripe_payment_webhook(request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    try:
        ...
        if event["type"] == "checkout.session.completed":
            ...
            course_enrollment_payment_update(intent.get("id"), enrollment_update_data, db)
            payment_done = True
...
def user_enrollment_by_course_id(course_id: int, db: Annotated[Session, Depends(get_db)], user: Annotated[User, Depends(get_current_user)]):
    try:
...
def fetch_enrolled_courses(user: Annotated[User, Depends(get_current_user)], db: Annotated[Session, Depends(get_db)]):
    try:
